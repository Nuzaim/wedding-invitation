...
export const sqliteSheetCache: SheetCache = {
  readCachedRows(dataset: SheetDataset) {
  ...
  getLastSyncedAt(dataset: SheetDataset) {
    const db = getSqliteDb();
    const record = db
  ...
  isCacheFresh(dataset: SheetDataset) {
    return Date.now() - sqliteSheetCache.getLastSyncedAt(dataset) < getSqliteCacheTtlMs();
  },

  replaceCachedRows(dataset: SheetDataset, rows: Array<{ rowNumber: number; row: SheetRow }>) {
    const db = getSqliteDb();
  ...
  upsertCachedRow(dataset: SheetDataset, rowNumber: number, row: SheetRow) {
    const db = getSqliteDb();
    db.prepare(
      "INSERT INTO sheet_rows (dataset, row_number, row_json) VALUES (?, ?, ?) " +
  ...
  markCacheStale(dataset: SheetDataset) {
    const db = getSqliteDb();
    db.prepare(
